...
// ponytail: дедуплікація без стану — вікно часу замість справжнього стану.
// Шлемо лише о 04:0x UTC (07:0x за Києвом). UptimeRobot пінгує раз на 5 хв,
// тож у вікно потрапляє рівно один пінг — одне повідомлення на добу.
//
// Спершу тут була година замість доби, і за перший же збій (він триває до
// відновлення квоти, тобто тижнями) у групу пішло б ~350 повідомлень.
// Якщо колись знадобиться «один раз на збій» — потрібен стан (KV/Redis),
// вікном часу цього не досягти.
const NOTIFY_HOUR_UTC = 4;

function shouldNotify() {
  const d = new Date();
  return d.getUTCHours() === NOTIFY_HOUR_UTC && d.getUTCMinutes() < 5;
}
