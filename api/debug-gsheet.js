// ВРЕМЕННЫЙ — діагностика структури таблиці + прибирання тестового рядка. Видалити після тесту.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const meta = await (await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${SS}?fields=sheets.properties`,
      { headers: { Authorization: 'Bearer ' + token } }
    )).json();
    const sheet = meta.sheets.find(s => String(s.properties.sheetId) === '1059855693');
    const title = sheet.properties.title;

    const range = req.query.range || `'${title}'!A1:W16`;
    ...
    if (req.query.clear === '1') {
      const clearRange = `'${title}'!A15:W15`;
      cleared = await (await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/${SS}/values/${encodeURIComponent(clearRange)}:clear`,
        { method: 'POST', headers: { Authorization: 'Bearer ' + token } }
      )).json();
    }

    res.status(200).json({ title, sheetId: sheet.properties.sheetId, values: vals.values || [], cleared });
  } catch (e) {
